var caculateMap = function (mapArray) {
    var parentArray = []
    var childArray = []
    var rootNode;
    mapArray.forEach(node => {
        var childParent = node.split(')');
        var parent = childParent[0];
        var child = childParent[1];
        parentArray.push(parent);
        childArray.push(child);
    });

    var generateChildren = function (node, remainingParents, remainingChildren) {
        var index = _.indexOf(remainingParents, node.label);
        while (index >= 0) {
            var newNode = new Planet(remainingChildren[index]);
            node.addChild(newNode);
            remainingParents.splice(index, 1);
            remainingChildren.splice(index, 1);
            index = _.indexOf(remainingParents, node.label);
        }
        node.children.forEach(function (child) {
            generateChildren(child, remainingParents, remainingChildren);
        });
    }

    var startNode = _.difference(parentArray, childArray);
    var rootNode = new Planet(startNode[0], null);
    generateChildren(rootNode, parentArray, childArray);
...
class Planet {
    ...
    numberOfParents() {
        if (this.parent) {
            return 1 + this.parent.numberOfParents();
        }
        else {
            return 0;
        }
    ...
    numberOfOrbits() {
        ...
        this.children.forEach(planet => {
            orbits += planet.numberOfOrbits() + planet.numberOfParents();
        });
        return orbits;
    }
    getMainParent() {
