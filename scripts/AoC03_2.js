var calculatePaths = function (line1, line2, grid) {
    var line1Array = line1.split(',');
    var line2Array = line2.split(',');
    var cursor;
    var steps;
    var moveLine = function (direction, amount, cursor, grid, lineValue) {
        var i;

        var setGridValue = function (grid, cursor, steps) {
            if (typeof grid[cursor[0]] === 'undefined') {
                grid[cursor[0]] = {};
            }
            //On first visit, set steps.
            if (typeof grid[cursor[0]][cursor[1]] === 'undefined') {
                grid[cursor[0]][cursor[1]] = { value: 0 };
            }
            //On first visit, set steps.
            if (typeof grid[cursor[0]][cursor[1]]['line' + lineValue] === 'undefined') {
                grid[cursor[0]][cursor[1]]['line' + lineValue] = steps;
            }

            if (grid[cursor[0]][cursor[1]].value < lineValue) {
                grid[cursor[0]][cursor[1]].value += lineValue
            }
        };
        ...
        switch (direction) {
            case 'U':
                for (i = 0; i < amount; i++) {
                    ...
                    cursor[1]++;
                    steps++;
                    setGridValue(grid, cursor, steps);
                }
                break;
            case 'D':
                for (i = 0; i < amount; i++) {
    ...
    var calculateDistance = function (grid, value) {
        var distance = 100000;
        Object.keys(grid).forEach(function (x) {
            Object.keys(grid[x]).forEach(function (y) {
                if (grid[x][y].value >= value) {
                    ...
                    distance = Math.min((distX + distY), distance);
                    console.log('x: %d, y: %d -> distance: %d', distX, distY, distX + distY);
                }
            });
        });
        return distance;
    ...
    var calculateSteps = function (grid, value) {
        var steps = 100000;
        Object.keys(grid).forEach(function (x) {
            Object.keys(grid[x]).forEach(function (y) {
                if (grid[x][y].value >= value) {
                    //We have a junction, so line1 and line2 are defined.
                    steps = Math.min((grid[x][y].line1 + grid[x][y].line2), steps);
                    //console.log('steps: %d', grid[x][y].line1 + grid[x][y].line2);
                }
            });
        });
        return steps;
    ...
    cursor = [0, 0];
    steps = 0;
    line1Array.forEach(function (instruction) {
        //console.log(instruction);
        var direction = instruction[0];
        var amount = instruction.substr(1);
        moveLine(direction, amount, cursor, grid, 1);
    });

    //reset cursor
    cursor = [0, 0];
    steps = 0;
    line2Array.forEach(function (instruction) {
        //console.log(instruction);
        var direction = instruction[0];
        var amount = instruction.substr(1);
        moveLine(direction, amount, cursor, grid, 2);
    });

    return calculateSteps(grid, 3);
}
...
console.log("steps: %d", calculatePaths(line1, line2, grid));
